...
%%%%%%%%%%%%%%if the lines are almost parallel, use centers as contact
%%%%%%%%%%%%%%points
        x1 = [(p1(1) + p2(1))/2 (p1(2) + p2(2))/2];
        x2 = [(p3(1) + p4(1))/2 (p3(2) + p4(2))/2];
        X = [(x1(1) - x2(1)) (x1(2) - x2(2))];
        distance = norm(X); 
        varargout(1) = {X};   % vector connecting the closest points
        varargout(2) = {x1};   % Closest point on object 1 
        varargout(3) = {x2};   % Closest point on object 2
    return
